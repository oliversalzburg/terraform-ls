...
func (w *Walker) SetWatcher(watcher watcher.Watcher) {
	w.watcher = watcher
}
...
func (w *Walker) walk(ctx context.Context, rootPath string) error {
	...
	err := filepath.Walk(rootPath, func(path string, info os.FileInfo, err error) error {
		...
		if info.Name() == datadir.DataDirName {
			...
			if err != nil {
				w.modMgr.AddModuleAtPath(dir)
				return err
			}

			w.modMgr.EnqueueModuleOp(dir, OpTypeGetTerraformVersion)

			dataDir := datadir.WalkDataDirOfModule(w.fs, dir)
			if dataDir.ModuleManifestPath != "" {
				w.modMgr.EnqueueModuleOp(dir, OpTypeParseModuleManifest)
			}
			if dataDir.PluginLockFilePath != "" {
				w.modMgr.EnqueueModuleOp(dir, OpTypeObtainSchema)
			}

			if w.watcher != nil {
				w.watcher.AddModule(dir, dataDir)
			}
